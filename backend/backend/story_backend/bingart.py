...
class BingArt:
    ...
    def generate_multiple_images(self, prompts):
        results = []
        for prompt in prompts:
            try:
                result = self.generate_images(prompt)
                results.append({'prompt': prompt, 'images': result['images'][:1]})  # Limit to 1 image per prompt
            except PromptRejectedError:
                results.append({'prompt': prompt, 'error': 'Prompt rejected for ethical reasons.'})
            except AuthCookieError:
                results.append({'prompt': prompt, 'error': 'Authentication failed.'})
            except Exception as e:
